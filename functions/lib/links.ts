...
import { monthlyClickQuota, retainedDays, effectiveTier, canExportCSV } from './subscription';
...
export const PRO_MONTHLY_CLICKS = 100000;
...
export async function accountUsage(db: D1Database, developerId: string): Promise<any> {
  const u = await db.prepare(
    `SELECT subscription_tier, subscription_expires_at, ever_pro, link_click_month, link_click_count FROM users WHERE id = ?`
  ).bind(developerId).first<any>();
  const mk = monthKey();
  if (!u) return { limit: FREE_MONTHLY_CLICKS, used: 0, month: mk, tier: 'free', retention_days: 30, can_export_csv: false };
  // 用有效等级（过期 Pro 回落 free），与 recordClick 的口径保持一致
  const tier = effectiveTier(u);
  // 保留天数认「现在是或曾是 Pro」：降级用户仍按 90 天保留，不显示成 30 天
  const retention = retainedDays(u, 'link');
  const used = u.link_click_month === mk ? (u.link_click_count || 0) : 0;
  return {
    limit: monthlyClickQuota(tier),
    used,
    month: mk,
    tier,
    retention_days: retention,
    can_export_csv: canExportCSV(tier),
    ever_pro: !!(u.ever_pro || u.subscription_tier === 'pro'),
  };
}
